...
if __name__ == '__main__':
  while True:
    ...
    rpm=int(info.physics.rpms)
    i=int(info.physics.speedKmh)
    f=int(info.physics.fuel)
    g=int(info.physics.gear)
    mr=int(info.static.maxFuel)
    mrr=int(info.static.maxRpm)
    abs1=float(info.physics.brake)
    tc1,tc2,tc3,tc4=info.physics.wheelSlip
    x,y,z=info.physics.accG
    tc1=tc1*100
    abs1=abs1*100
    tc1=int(tc1)
    abs1=int(abs1)
    ...
    arduino.write(str.encode(str(rpm)+"A"+str(i)+"B"+str(u)+"C"+str(tc1)+"D"+str(abs1)+"E"+str(xx)+"F"+"\n"))
